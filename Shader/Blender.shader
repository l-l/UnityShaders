Shader "Custom/Blender" {
	Properties {
		...
		_NormalVal("Normal Value",Range(0.0,1.0))=1.0
...
#include "UnityCG.cginc"
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 occlusion = tex2D(_OcclusionMap, IN.uv_DissolveMap);// *_OcclusionVal;
			...
			o.Alpha = c.a;
			fixed4 normal = tex2D(_NormalMap, IN.uv_MainTex);
			
			o.Normal = UnpackScaleNormal(normal,_NormalVal);// UnityObjectToWorldNormal(normal.xyz) *_NormalVal;
			o.Occlusion = lerp(occlusion.r,1.0,1.0-_OcclusionVal);// *(1.0 - _OcclusionVal);
